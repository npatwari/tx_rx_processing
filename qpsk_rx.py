...
# For having correct count expected symbols
temp = 'I worked all semester on digital communications and all I got was this sequence of ones and zeros.'
messagebits  = text2bits(temp)
data = binary2mary(messagebits, 4)

# Read from File

s = get_samps_from_file('QPSK_signal2.iq')
# Down-convert
...
# Testing for symbol synchronization through observing
# plotted signal yI and yQ
n = list(range(0, len(s_rx_I)))
n = np.array(n)
t = n.reshape((len(s_rx_I),))
...
# Original symbol synchronization offsets
# y_s_I = yI[0][96:]
# y_s_Q = yQ[0][96:]

# Tested symbol synchronization offsets
# y_s_I = yI[82:]
# y_s_Q = yQ[82:]
# y_s_I = yI[248:]
# y_s_Q = yQ[248:]
...
y_s_I = yI[360:]
y_s_Q = yQ[360:]
# Extra 'W' character at beginning of message and missing 's.' at the end.
# Eye diagram looks good like the 96 from previous messages.
...
# # Testing for symbol synchronization offset with eye diagrams
# for off in range(75,110,7):
#     y_s_I = yI[off:]
#     y_s_Q = yQ[off:]
#     hI_temp = plot_eye_diagram(y_s_I, 8, 0)
#     hQ_temp = plot_eye_diagram(y_s_Q, 8, 0)
...
for k in range(length):
    ...
    if count == US_Rate:
        ...
        count = 0
        h = h + 1
expected_symbols = len(data) # chr len (message) * 7 / 2
